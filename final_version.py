...
def plot_map(
  ...
  gdf = ox.features.features_from_polygon(polygons.geometry.iloc[0], tags=tags)
  ...
  if len(land_patches):
    land_patches["color"] = [mcolors.to_rgba(c, alpha=0.4) for c in np.random.choice(palette, len(land_patches))]
  ...
  if len(streets):
    ...
    main_roads = streets[streets["highway"].isin(["primary", "trunk", "motorway"])]
    secondary_roads = streets[~streets["highway"].isin(["primary", "trunk", "motorway"])]
    if len(secondary_roads):
      secondary_roads.plot(ax=ax, color="#3a3a3a", linewidth=0.5, alpha=1)
    if len(main_roads):
      main_roads.plot(ax=ax, color="#3a3a3a", linewidth=1.1, alpha=0.9)

  if len(buildings):
    buildings.plot(ax=ax, color=buildings["color"], edgecolor="none", linewidth=0, alpha=0.8)
  ...
  add_two_lines_text(fig, "Mainz, Germany", decimal_to_dms(lat, lon), posx=0.5, posy=0.13)
...
if __name__ == "__main__":
  ...
  print("Map saved to mainz_final.png")